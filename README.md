...
The 17 IDF files included represent several U.S. Department of Energy reference archetypes extracted from Building Technology Assessment Platform (BTAP), which is an open-source building energy simulation tool developed by Natural Resources Canada. The IDF files have been adjusted such that they work with the SimRunner package, and are no longer compatible with the original tool. For more information regarding the complementary IDF input files, please look into BTAP’s resources page.
